...
from method import Random, LinUCBAlgorithm, CLUBAlgorithm, CoLinUCBAlgorithm, GOBLinAlgorithm, Hybrid_LinUCBAlgorithm
...
def save_stdev():
    with open('../analytics/stdev/usercluster200.csv', 'w') as f:
        for i in range(200):
            for id_, val in enumerate(alg.users[i].stdev_context):
                f.write(str(val))
                if id_ != len(alg.users[i].stdev_context)-1:
                    f.write(',')
                else:
                    f.write('\n')
...
def gragh(ctr_list, reward_list):
    for name, val in ctr_list.items():
        if name == 'OriginalUCB':
    ...
    for name, val in reward_list.items():
        if name == 'OriginalUCB':
            plt.plot([i for i in range(len(val))], val, marker='o')
        else:
            plt.plot([i for i in range(len(val))], val, marker='v')
    plt.grid(True)
    plt.show()
        ...
        with open(FILE_DIR + file_name) as f:
            for line in f:
                ...
                for name, alg in algorithms.items():
                    ...
                    if name == 'OriginalUCB' and iteration < 150000 and random.random() < (1.- (iteration*0.5/150000)):
...
if __name__ == "__main__":
    ...
    algorithms['Hybrid_LinUCB'] = Hybrid_LinUCBAlgorithm(dimension, alpha, lambda_)
